...
import { TREBLE_TOP, BASS_TOP, STAFF_HEIGHT, STAFF_AREA_PAD, DEAD_ZONE_H } from '../notation/layout';
...
interface Scale { sx: number; sy: number; rect: DOMRect; oX: number; oY: number; vbW: number; vbH: number; }

// ── Factory ───────────────────────────────────────────────────────────────────

export function createTouchContext(options?: { flipped?: boolean; sharedCooldown?: { until: number }; compact?: boolean }) {
  const flipped = options?.flipped ?? false;
  const compact = options?.compact ?? false;
  const cooldown = options?.sharedCooldown ?? { until: 0 };
  ...
  // ── Zone helpers (derived from STAFF_AREA_PAD / DEAD_ZONE_H / compact) ────
  //
  // Solo zones (top → bottom):
  //   tap-advance  → [viewportTop, topDeadStart]
  //   dead zone    → [topDeadStart, firstStaffAreaTop]   = [staffTop[0]-STAFF_AREA_PAD-DEAD_ZONE_H, staffTop[0]-STAFF_AREA_PAD]
  //   staff areas  → staffTop[i]-STAFF_AREA_PAD .. staffTop[i]+STAFF_HEIGHT+STAFF_AREA_PAD
  //   dead zone    → [lastStaffAreaBottom, lastStaffAreaBottom+DEAD_ZONE_H]
  //   tap-advance  → [lastStaffAreaBottom+DEAD_ZONE_H, viewportBottom]
  //
  // 2-player / compact zones (divider=top → outer edge=bottom):
  //   staff areas  → 0 .. staffCount*STAFF_AREA_H
  //   dead zone    → [lastStaffAreaBottom, lastStaffAreaBottom+DEAD_ZONE_H]
  //   tap-advance  → [lastStaffAreaBottom+DEAD_ZONE_H, viewportBottom]

  function topDeadZoneRange(): [number, number] {
    const firstTop = currentStaffTops[0] ?? TREBLE_TOP;
    return [firstTop - STAFF_AREA_PAD - DEAD_ZONE_H, firstTop - STAFF_AREA_PAD];
  }

  function botDeadZoneRange(): [number, number] | null {
    const n = currentStaffTops.length;
    const lastTop = currentStaffTops[n - 1] ?? TREBLE_TOP;
    const lastAreaBottom = lastTop + STAFF_HEIGHT + STAFF_AREA_PAD;
    return [lastAreaBottom, lastAreaBottom + DEAD_ZONE_H];
  }

  function botTapAdvanceStart(): number {
    const n = currentStaffTops.length;
    const lastTop = currentStaffTops[n - 1] ?? TREBLE_TOP;
    const lastAreaBottom = lastTop + STAFF_HEIGHT + STAFF_AREA_PAD;
    return lastAreaBottom + DEAD_ZONE_H;
  }
  ...
  function svgScale(svg: SVGSVGElement): Scale {
    ...
    return { sx: vb.width / cW, sy: vb.height / cH, rect, oX, oY, vbW: vb.width, vbH: vb.height };
  }

  function clientToSVG(e: { clientX: number; clientY: number }, sc: Scale) {
    const x = (e.clientX - sc.rect.left - sc.oX) * sc.sx;
    const y = (e.clientY - sc.rect.top  - sc.oY) * sc.sy;
    if (flipped) {
      return { x: sc.vbW - x, y: sc.vbH - y };
    }
    return { x, y };
  }
  ...
  function isTapAdvanceAreaY(svg: SVGSVGElement, _staff: number, touchY: number): boolean {
    const vb = svg.viewBox.baseVal;
    const sc = svgScale(svg);
    const viewportBottom = vb.height + sc.oY * sc.sy;

    // Bottom tap-advance (both modes)
    if (touchY >= botTapAdvanceStart() && touchY < viewportBottom) return true;

    // Top tap-advance (solo only — in compact/2-player mode there is no top tap-advance)
    if (!compact) {
      const [deadStart] = topDeadZoneRange();
      const viewportTop = -sc.oY * sc.sy;
      if (deadStart > 0 && touchY >= viewportTop && touchY < deadStart) return true;
    }
  ...
  function activateBeatDirect(
    ...
    if (n > 0) {
      ...
      const base  = computeGain(touchH, true);
  ...
  function updateStaff(
    ...
    if (ss.activeBeat !== beatPrefix) {
      ...
      for (const [noteId, handle] of [...ss.activeNotes]) {
        if (ss.carriedHandlePitches.has(noteId)) continue;
        const t = targets.find(t2 => t2.noteId === noteId);
        if (t && tiedForward.has(t.midiPitch)) continue;
        handle.stop();
        setColor(svg, noteId, COLOR_DEFAULT);
  ...
  // Register global transition cleanup once per context
  ...
  function attachTouchHandlers(
    ...
    function inDeadZone(y: number): boolean {
      // Top dead zone (both modes)
      const [topDStart, topDEnd] = topDeadZoneRange();
      if (y >= topDStart && y < topDEnd) return true;
      // Bottom dead zone (both modes)
      const botDZ = botDeadZoneRange();
      if (botDZ && y >= botDZ[0] && y < botDZ[1]) return true;
      return false;
    }
    ...
    svg.addEventListener('pointermove', (e: PointerEvent) => {
      ...
      if (transitions.has(e.pointerId)) {
        ...
        tr.lastX = x; tr.lastY = y;
        upsertIndicator(svg, e.pointerId, x, y, e.width ?? 1, e.height ?? 1);
        // For flipped context, "right edge" is visually on the left — check clientX from left
        const atEdge = flipped
          ? e.clientX <= nextBarZoneW()
          : e.clientX >= window.innerWidth - nextBarZoneW();
        const pastEdge = flipped
          ? e.clientX > nextBarZoneW() + 10
          : e.clientX < window.innerWidth - nextBarZoneW() - 10;
        if (!tr.triggeredNextBar && atEdge) {
          tr.triggeredNextBar = true;
          if (Date.now() >= cooldown.until) {
            cooldown.until = Date.now() + 2000;
            onNavigateNext?.();
          }
          return;
        }
        if (tr.triggeredNextBar && pastEdge) {
      ...
      if (st.triggeredNextBar && pastEdge) {
      ...
      if (!st.isTapAdvance) {
        const targets = getTargets();
        if (inDeadZone(y)) { st.wiggle.update(e.clientX); return; }
        for (const s of staffNums()) {
  ...
  // ── captureForTransition ──────────────────────────────────────────────────

  function captureForTransition(currentTargets: HitTarget[] = []): number[] {
  ...
  function stopAllNotes(svg?: SVGSVGElement) {
    pendingRingOut.forEach(h => h.stop());
    pendingRingOut.clear();
    for (const [id, st] of pointers.entries()) {
  ...
  function getActivePointerIds(): number[] { return [...pointers.keys()]; }

  // ── drawDebugZones ────────────────────────────────────────────────────────

  function drawDebugZones(svg: SVGSVGElement, _targets: HitTarget[]): void {
    ...
    function rect(y1: number, y2: number, color: string, opacity = 0.3) {
      if (y2 <= y1) return;
    ...
    // Top dead zone (transparent — no fill)
    // const [topDStart, topDEnd] = topDeadZoneRange();

    // Top tap-advance (grey, solo only)
    const [topDStart] = topDeadZoneRange();
    if (!compact && topDStart > viewportTop) {
      rect(viewportTop, topDStart, '#f8f8f6', 1);
    }

    // Bottom tap-advance (grey)
    rect(botTapAdvanceStart(), viewportBottom, '#f8f8f6', 1);

    // Insert after the white background rect so zones are visible, but before score content
    svg.insertBefore(g, svg.firstChild?.nextSibling ?? null);
  }

  // ── Public API ────────────────────────────────────────────────────────────

  return { attachTouchHandlers, captureForTransition, stopAllNotes, getActivePointerIds, drawDebugZones };
}
